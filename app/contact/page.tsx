...
export default function ContactPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // THESE ARE YOUR EMAILJS KEYS (See Step 3 below on where to get them)
    // For production, put these in a .env.local file: process.env.NEXT_PUBLIC_SERVICE_ID
    const serviceID = "service_b1mosif";
    const templateID = "template_w3z2ljd";
    const publicKey = "3yetahzWvgq09WKEX";

    // Create a concise object that matches your EmailJS template variables
    const templateParams = {
      from_name: formData.name,
      from_email: formData.email,
      subject: formData.subject,
      message: formData.message,
      to_email: "vanta83@gmail.com" // You can also hardcode this in the EmailJS dashboard
    };
          ...
          <div className="container mx-auto px-6 max-w-6xl">
            <div className="grid md:grid-cols-3 gap-6 mb-16">
              <div className="p-8 bg-black/40 border border-white/10 rounded-xl text-center hover:border-[#00f3ff]/50 transition-all">
                ...
                <a 
                  href="mailto:support@autobuyguard.com" 
                  className="text-[#00f3ff] hover:underline text-sm"
                >
                  support@autobuyguard.com
                </a>
              </div>
